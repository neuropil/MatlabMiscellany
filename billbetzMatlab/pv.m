   ...
   if strcmp(inp,'Cancel'); return; end
      ...
      case 'imgprocesspopup'
        ...
        switch val
          ...
          case 14 % sort images by mean brightness
            bkg=min(v.Movi(:)); c=zeros(size(v.Movi,3),1); v.Movi2=v.Movi;
            ...
            [d idx]=sortrows(c,-1);
            for j=1:size(v.Movi,3)
              v.Movi2(:,:,j)=v.Movi(:,:,idx(j,1));
            end
            setappdata(vh.fig,'vh',vh); setappdata(vh.fig,'v',v)
            v0.callingfig=vh.fig; setappdata(0,'v0',v0)
            eval([mfilename ' sort_by_avgBri' ' sort_by_avgBri' ' sort_by_avgBri'])
            return
...
try
movobj=VideoReader([v.picdir v.list{1}]);
...
for k=1:nFrames
  v.Movi(:,:,:,k)=read(movobj,k);
  v.list(k)={num2str(k)};
end
v.bitdepth=8; v.pse=zeros(1,nFrames); v.rgbyes=1;
set(vh.bitdepth,'string',[num2str(v.bitdepth) ' bits'])
v.pse=1:nFrames; v.pse=v.pse.*0;
v.Movi2=v.Movi;
setappdata(gcf,'v',v); setappdata(0,'v0',v0)
return
catch; disp('NOT A VIDEO'); end
