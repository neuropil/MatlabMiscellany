...
switch nargin
  case 0
    ...
    v.pthreshtau=5; v.fitstr={}; v.newrrp=0; v.slctall=0; v.update=0;
  ...
  case 1
    switch varargin{:}
      ...
      case 'optim_cover'
        set(vh.optim_cover,'visible','off')
      case 'update'
        v.update=~v.update;
      case 'objfx'
      ...
      case 'histogram' % current distribution of p values
        ...
        set(vh.axes1,'xlim',[0 1],'ylim',[0 mx])
       % set(vh.axes1,'xlim',[0 1],'ylimmode','auto')
         set(vh.axes1,'ButtonDownFcn',[mfilename ' axes_scale'])
        drawnow
...
if ~mod(nn,50); set(vh.counter,'string',num2str(nn)); disp([v.optimctr nn]); end
...
rtaustd=0.3; rnd0=rand(size(v.n,1),1);
rtau0=randn(size(v.n,1),1)*rtaustd+v.rtau;
%rtau0(rtau0<0.001)=0.001+abs(rtau0(rtau0<0.001));
rtau0(rtau0<0.001)=rnd0(rtau0<0.001);
fac=v.rtau/mean(rtau0);
rtau0=max(0.001,rtau0*fac);
v.rtau=mean(rtau0);
v.rn=round(min(size(v.n,1),v.rn));
%rtau0=sortrows(rtau0,1);
if v.rn<size(v.n,1)
  rtau0(v.rn+1:end,:)=1000;  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
%rate0=rtau0.^-1;
%set(findobj('tag','rtau_txt'),'string',['rtau= ' num2str(mean(rtau0))])
%set(findobj('tag','rtau_slider'),'value',(mean(rtau0)))
%set(findobj('tag','rtau_txt'),'string',['rtau= ' num2str(mean(rate0))])
%set(findobj('tag','rtau_slider'),'value',(mean(rate0)))
...
set(vh.line10,'xdata',v.n(:,3),'ydata',rtau0,...
