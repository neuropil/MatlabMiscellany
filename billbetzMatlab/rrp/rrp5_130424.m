...
switch nargin
  case 0
    ...
    v.rrpsize=1770; v.rrpavg=0.5; v.rrpstd=0.2; v.rrpskew=1;
    v.rtau=.2; v.rskew=1; v.deadtime=0; v.rthresh=0;
    v.rrpslow=1; v.recruitslow=1; v.pthreshtau=5;
    % bounds: v.rtau, rskew, deadtime, rthresh, rrpslow, recruitslow
    v.lb=[.01 .01 0 0 .1 .1];
    v.ub=[.5 10 1 1 10 10];
    v.mx0=1; % max value to which p0 can grow
    v.npts=100; v.freq=100; v.repeat=10;
    ...
    switch varargin{:}
      ...
      case 'vars'
        ...
        prompt={'RRP size'  'RRP p (mean)' 'RRP p (std dev)',...
          ...
          'noise factor?' 'max to which p0 can grow' 'pthreshtau'};
        ...
        v.recruitslow=str2num(inp{10});
        
        v.npts=str2num(inp{11});
        v.noisefac=str2num(inp{12});
        v.mx0=min(1,str2num(inp{13}));
        v.pthreshtau=str2num(inp{14});
      ...
      case 'getvals' % all sliders
        ...
        v.recruitslow=get(vh.recruitslow_slider,'value');
      ...
      case 'setvals'
        ...
        set(vh.recruitslow_txt,'string',['recyc slow=' num2str(v.recruitslow)])
        ...
        set(vh.recruitslow_slider,'value',v.recruitslow)
      ...
      case 'rrpgrow'
        ...
        set(vh.rrpslow_txt,'visible',str2)
        set(vh.rrpslow_slider,'visible',str2)
        if ~v.setup; eval([mfilename ' setup_rrp']); end
        if v.repeat>1; return; end
        opt
        eval([mfilename ' histogram'])
      ...
      case 'optim_bounds'
        prompt={'recruit tau lower bound' 'recruit tau UPPER bound',...
          ...
          'rrpslow lower bound' 'rrpslow UPPER bound',...
          'recycle slow lower bound' 'recycle slow UPPER bound'}';
      ...
      case 'optim'
        ...
        x=[v.lb(1) v.rtau v.ub(1),...
          ...
          v.lb(5) v.rrpslow v.ub(5),...
          v.lb(6) v.recruitslow v.ub(6)];
      ...
      case 'setup_rrp'
        ...
        v.rthresh2=z(round(m1/mean(z(1:m1)))); % p value of mth AZ
...
global v vh
if ~v.optim;
  x=[v.rtau v.rskew v.deadtime v.rthresh v.rrpslow v.recruitslow];
end
...
if v.optim
  ...
  v.rrpslow=x(5); v.recruitslow=x(6);
end
...
v.mnow=[];
%x=[0:v.res:v.res*10000]';
%v.exp=1-exp(-x/(v.rtau)); % lookup table for expl rise of p
%if v.rskew~=1;
%  v.exp=v.exp.^v.rskew; % skew expl
%end
v.rectime=[];
...
v.rthresh0=z(round(m1/mean(z(1:m1)))); % avg p value of first m1 responses

%v.pthreshtau=4; % tau (#pts) at which p threshold decays from v.rthresh0 to v.rthresh
x=[0:v.npts-1]';
...
for j=1:v.npts %%%%%%%%%%%%%%%%%%%%%
  ...
  df=1-exp(-dtms/v.rtau); % fractional rise of p
  if v.rskew~=1;
    df=df.^v.rskew; % skew expl
  end
  a0=v.n(:,2)+1; a=a0; % determine how much to slow the fractional increase in p
  a(a0==1)=a(a0==1)*v.rrpslow; % slow p increase of virgins
  a(a0>1)=a(a0>1)*v.recruitslow; % slow recycle rate with multiple exocytoses
  a(a0>1)=a(a0>1).*(a(a0>1)-1);
  df=df./a;
  p=mn+y0.*df;
  noisey=(rand(size(v.n,1),1)-0.5)*2; % range is -1 to +1
