...
switch nargin
  ...
  %  v.rtau=.2; v.rskew=0; v.deadtime=0; v.rthresh=0;
    v.pthreshtau=5; v.fitstr={}; v.newrrp=0;
    ...
    switch varargin{:}
      ...
      case v.varlist_slider % get values of all sliders
        ...
        for j=1:size(v.var,1)
          ...
          eval(['v.' v.var{j,1} '=round(a.Value*100)/100']);
        end
        eval([mfilename ' setvals'])
        v.optim=0;
        ...
        for j=1:size(v.var(:,1))
         ...
         if val>=vv.Max || val<=vv.Min
           dv=vv.Max-vv.Min; % range doesn't change
            vv.Min=max(0,val-0.5*dv); vv.Max=val+0.5*dv;
...
%dead0=randn(size(v.n,1),1)*v.deadtime+v.deadtime; dead=abs(dead0); %dead(dead0>1)=dead(dead>1)-1;
dead0=rand(size(v.n,1),1)*v.deadtime+v.deadtime;
...
for j=1:v.npts %%%%%%%%%%%%%%%%%%%%%
  ...
  dtms=max(0,tnow-dead-v.n(:,1)); % length of time that p has been growing
