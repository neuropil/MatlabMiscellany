...
if ~options.griddedInput
    % Scattered query points were provided
    [unqQpts,~,unqQPtInds] = unique(qPts(:,1:2),'rows');
    unqQptIndsCell = accumarray(unqQPtInds,1:length(unqQPtInds), [],@(x){x});
    qPtsViaUnqIndice = @(ind)qPts(unqQptIndsCell{ind},:);
    outPxIndsViaUnqIndiceMask = @(ind,mask)unqQptIndsCell{ind}(mask);
    outputSize = [size(qPts,1),1];
else
    % Structured query points were provided.
    ...
    qPtsViaUnqIndice = @(ind)bsxfun(@plus, zCoords, [unqQpts(ind,:) 0]);
...
for i = 1:length(newInds)-1
    ...
    for ptNo = find(qPtHitsTriangles(:))'
        % Which of the unique "2D" points are we querying?
        unqQptInd = fromTo(ptNo);
        ...
        facetToQptVectors = bsxfun(@minus, qPtsViaUnqIndice(unqQptInd), facets(1,:,piercedFacetInds));
...
if options.griddedInput
    IN = reshape(IN', cellfun(@numel, qPts([2 1 3])));
end
